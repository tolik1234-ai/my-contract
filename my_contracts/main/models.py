...
class ContractDeployment(models.Model):
    ...
    deployer_wallet = models.CharField(
        max_length=128,
        blank=True,
    ...
    manager_address = models.CharField(max_length=120, blank=True)
    deployment_metadata = models.JSONField(default=dict, blank=True)
    ...
    def mark_success(
        ...
        if metadata:
            self.deployment_metadata = metadata
        if raw_output:
